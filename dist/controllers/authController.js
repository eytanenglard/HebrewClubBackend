...
export const csrfTokenHandler = (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Error generating CSRF token', error: error.message });
    }
};
...
export const verifyToken = async (req, res) => {
    ...
    const existingToken = req.cookies.accessToken || ((_a = req.header('Authorization')) === null || _a === void 0 ? void 0 : _a.replace('Bearer ', ''));
    ...
    try {
        ...
        if (existingToken) {
            try {
                const existingDecoded = jwt.verify(existingToken, JWT_SECRET);
...
export const refreshToken = async (req, res) => {
    ...
    const token = req.cookies.accessToken || req.body.token || ((_a = req.header('Authorization')) === null || _a === void 0 ? void 0 : _a.replace('Bearer ', ''));
    if (!token) {
...
const sendEmailVerification = async (to, verificationToken, verificationCode, name) => {
