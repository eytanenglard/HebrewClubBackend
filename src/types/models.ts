...
// Helper type for converting ObjectId to string
...
export interface EditableUserProfile {
  ...
  bio?: string;
  avatar?: string;
  role?: string;  // הוספנו את זה
  createdAt?: string;  // הוספנו את זה
  updatedAt?: string;  // הוספנו את זה
  twoFactorEnabled?: boolean;  // הוספנו את זה
  isEmailVerified?: boolean;  // הוספנו את זה
  status?: 'active' | 'inactive' | 'locked';  // הוספנו את זה
  failedLoginAttempts?: number;  // הוספנו את זה
}

// אין צורך ב-EditableUserProfileResponse יותר, כי EditableUserProfile כבר כולל _id
...
export type InstructorInfoResponse = WithStringId<Omit<InstructorInfo, 'userId'>> & { userId: string };
...
export type CertificateResponse = Omit<Certificate, 'courseId'> & { courseId: string };
...
export type ContentItemResponse = WithStringId<Omit<ContentItem, 'lessonId'>> & { lessonId: string };
...
export type QuizQuestionResponse = WithStringId<Omit<QuizQuestion, 'quizId'>> & { quizId: string };
...
export type AssignmentResponse = WithStringId<Omit<Assignment, 'courseId'>> & { courseId: string };
...
export type LeadResponse = WithStringId<Omit<Lead, 'courseInterest'>> & {
  courseInterest: string[]; // This will be an array of string IDs in the response
};
